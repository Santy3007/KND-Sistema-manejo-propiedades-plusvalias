...
namespace KNDMovil.Services
{
    public class PropiedadService
    {
        ...
        private const string BaseUrl = "http://192.168.100.60/knd/webservices/";
        ...
        public async Task<string> CrearPropiedadAsync(Propiedad propiedad)
        {
            try
            {
                ...
                string url = $"{BaseUrl}getPropetiesCrud.php?action=crear";
        ...
        public async Task<string> ActualizarPropiedadAsync(Propiedad propiedad)
        {
            try
            {
                ...
                string url = $"{BaseUrl}getPropetiesCrud.php?action=actualizar";
        ...
        public async Task<string> EliminarPropiedadAsync(int proId)
        {
            try
            {
                ...
                string url = $"{BaseUrl}getPropetiesCrud.php?action=eliminar";
                Console.WriteLine($"🌍 URL: {url}");
        ...
        public async Task<List<Provincia>> GetProvinciasAsync()
        {
            try
            {
                var url = $"{BaseUrl}getPropetiesCrud.php?action=listar_provincias";
                var response = await _httpClient.GetStringAsync(url);
                var result = JsonConvert.DeserializeObject<ApiResponse<List<Provincia>>>(response);
        ...
        // Obtener ciudades por provincia_id
        public async Task<List<Ciudad>> GetCiudadesByProvinciaIdAsync(int provinciaId)
        {
            try
            {
                var url = $"{BaseUrl}getPropetiesCrud.php?action=listar_ciudades&provincia_id={provinciaId}";
                var response = await _httpClient.GetStringAsync(url);
                var result = JsonConvert.DeserializeObject<ApiResponse<List<Ciudad>>>(response);
