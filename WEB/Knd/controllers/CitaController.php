...
class CitaController {
    ...
    public function getEvents() {
        ...
        $per_id = ($_SESSION['rol_id'] == 1) ? null : $_SESSION['user_id'];
        $events = $this->citaModel->getEventsForCalendar($per_id);
    ...
    public function getById() {
        ...
        $cita = $this->citaModel->getById($_GET['id']);
        if ($cita) {
    ...
    public function create() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $citaId = $this->citaModel->create($data);
            if ($citaId) {
    ...
    public function updateDate() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['id'])) {
            ...
            $this->citaModel->update($id, ['cita_fecha' => $citaFecha]);
