...
<head>
    ...
    <title>PlusvaliaBlk - Editar Provincia</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800">Editar Provincia</h1>
                    <form action="index.php?controller=provincia&action=edit" method="POST">
                        <input type="hidden" name="provincia_id" value="<?php echo $provincia['provincia_id']; ?>">
                        <div class="form-group">
                            <label for="provincia_nombre">Nombre de la Provincia:</label>
                            <input type="text" name="provincia_nombre" class="form-control" value="<?php echo htmlspecialchars($provincia['provincia_nombre']); ?>" required>
                        </div>
                        <div class="form-group">
                            <button type="submit" class="btn btn-primary">Guardar</button>
                            <a href="index.php?controller=provincia&action=index" class="btn btn-secondary">Volver al Listado</a>
                        </div>
                    </form>
                        ...
                        <script>
                            document.addEventListener("DOMContentLoaded", function() {
                                Swal.fire({
                                    ...
                                    text: '<?= $success ? "✅ Provincia actualizada exitosamente." : "❌ Hubo un error al actualizar la provincia." ?>',
                                    icon: '<?= $success ? "success" : "error" ?>',
                                    confirmButtonText: 'Aceptar',
                                    confirmButtonColor: '<?= $success ? "#3085d6" : "#d33" ?>'
                                }).then(() => {
                                    if (<?= $success ? "true" : "false" ?>) {
                                        window.location.href = "index.php?controller=provincia&action=index";
                                    }
                                });
                            });
                        </script>
