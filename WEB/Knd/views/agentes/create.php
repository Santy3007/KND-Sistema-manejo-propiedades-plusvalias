...
function uploadFile($file) {
    if (!empty($file['name']) && $file['error'] == UPLOAD_ERR_OK) {
        $targetDir = __DIR__ . "/../../uploads/"; // Ruta absoluta a 'uploads/' en la raíz del proyecto
        ...
        if (move_uploaded_file($file["tmp_name"], $filePath)) {
            return "uploads/" . time() . '_' . basename($file["name"]); // Guardar ruta relativa para mostrarlo después
        }
    }
    return null;
}
    ...
    if (!empty($files['name'][0])) {
        foreach ($files['name'] as $key => $filename) {
            ...
            if (move_uploaded_file($files["tmp_name"][$key], $filePath)) {
                $uploadedPaths[] = "uploads/" . time() . '_' . basename($filename); // Guardar ruta relativa
            }
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        // Subir archivos
        $planosRuta = uploadFile($_FILES['pro_planos']);
        $imagenesRuta = uploadMultipleImages($_FILES['pro_imagenes']);
        ...
        $stmt = $pdo->prepare('INSERT INTO pro_propiedades 
        ...
        $stmt->execute([
            ...
            $_SESSION['user_id'] // Se guarda el ID del usuario autenticado
        ]);
                ...
                <div class="container-fluid">
                    ...
                    <form action="create.php" method="POST" enctype="multipart/form-data" class="form">
                        ...
                        <div class="form-group">
                            ...
                            <a href="http://localhost/knd/views/agentes/index.php" class="btn btn-secondary">Volver</a>
                        </div>
                    </form>
