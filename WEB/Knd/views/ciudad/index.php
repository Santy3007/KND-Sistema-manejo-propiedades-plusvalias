...
<head>
    ...
    <title>PlusvaliaBlk - Listado de Ciudades por Provincia</title>
...
<body id="page-top">
    <div id="wrapper">
        <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <li class="nav-item active"><a class="nav-link" href="index.php?controller=ciudad&action=index"><i class="fas fa-city"></i><span>Ciudades</span></a></li>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800">Listado de Ciudades por Provincia</h1>
                    <a href="index.php?controller=ciudad&action=create" class="btn btn-primary mb-3">Crear Nueva Ciudad</a>
                    <?php
                    try {
                        $stmt = $pdo->query("SELECT p.provincia_nombre, c.ciudad_id, c.ciudad_nombre FROM ciudades c JOIN provincias p ON c.provincia_id = p.provincia_id ORDER BY p.provincia_nombre, c.ciudad_nombre");
                        $ciudades = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    } catch (PDOException $e) {
                        echo "<p>Error al cargar las ciudades: " . $e->getMessage() . "</p>";
                        exit;
                    }
                    $provinciaActual = null;
                    foreach ($ciudades as $ciudad):
                        if ($ciudad['provincia_nombre'] != $provinciaActual):
                            if ($provinciaActual !== null): ?>
                                </ul>
                            <?php endif; ?>
                            <h2><?php echo htmlspecialchars($ciudad['provincia_nombre']); ?></h2>
                            <ul>
                        <?php 
                        $provinciaActual = $ciudad['provincia_nombre']; 
                        endif; 
                        ?>
                        <li>
                            <?php echo htmlspecialchars($ciudad['ciudad_nombre']); ?>
                            <div class="action-buttons">
                                <a href="index.php?controller=ciudad&action=edit&id=<?php echo $ciudad['ciudad_id']; ?>" class="btn btn-warning">Editar</a>
                                <button class="btn btn-danger btn-delete" data-id="<?php echo $ciudad['ciudad_id']; ?>">Eliminar</button>
                            </div>
                        </li>
                    <?php endforeach; ?>
                    </ul>
                                ...
                                button.addEventListener("click", function () {
                                    ...
                                    Swal.fire({
                                        ...
                                        if (result.isConfirmed) {
                                            window.location.href = `index.php?controller=ciudad&action=delete&id=${ciudadId}`;
                                        }
                                    });
                                });
                            });
