...
<head>
    ...
    <title>PlusvaliaBlk - Editar Ciudad</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
                ...
                <div class="container-fluid">
                    ...
                    <form action="index.php?controller=ciudad&action=edit" method="POST">
                        <input type="hidden" name="ciudad_id" value="<?php echo htmlspecialchars($ciudad['ciudad_id']); ?>">
                        <div class="form-group">
                            <label for="ciudad_nombre">Nombre de la Ciudad:</label>
                            <input type="text" name="ciudad_nombre" class="form-control" value="<?php echo htmlspecialchars($ciudad['ciudad_nombre']); ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="provincia_id">Provincia:</label>
                            <select name="provincia_id" class="form-control" required>
                                <?php foreach ($provincias as $provincia): ?>
                                    <option value="<?php echo $provincia['provincia_id']; ?>" <?php echo $ciudad['provincia_id'] == $provincia['provincia_id'] ? 'selected' : ''; ?>>
                        ...
                        <div class="form-group">
                            ...
                            <a href="index.php?controller=ciudad&action=index" class="btn btn-secondary">Volver al Listado</a>
                        </div>
                    </form>
                        ...
                        <script>
                            document.addEventListener("DOMContentLoaded", function() {
                                Swal.fire({
                                    ...
                                    text: "<?= $success ? '✅ Ciudad actualizada exitosamente.' : '❌ Hubo un error al actualizar la ciudad.'; ?>",
                                    icon: "<?= $success ? 'success' : 'error'; ?>",
                                    confirmButtonText: "Aceptar",
                                    confirmButtonColor: "<?= $success ? '#3085d6' : '#d33'; ?>"
                                }).then(() => {
                                    if (<?= $success ? 'true' : 'false'; ?>) {
                                        window.location.href = "index.php?controller=ciudad&action=index";
                                    }
                                });
                            });
                        </script>
