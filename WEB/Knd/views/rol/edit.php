...
<head>
    ...
    <title>PlusvaliaBlk - Editar Rol</title>
...
<body id="page-top">
    <div id="wrapper">
        <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <li class="nav-item active"><a class="nav-link" href="index.php?controller=rol&action=index"><i class="fas fa-user-shield"></i><span>Roles</span></a></li>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
                ...
                <div class="container-fluid">
                    ...
                    <form action="index.php?controller=rol&action=edit&id=<?php echo $rol['rol_id']; ?>" method="post">
                        <div class="form-group">
                            <label for="rol_nombre">Nombre del Rol:</label>
                            <input type="text" id="rol_nombre" name="rol_nombre" class="form-control" value="<?php echo $rol['rol_nombre']; ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="rol_status">Estado:</label>
                            <select id="rol_status" name="rol_status" class="form-control" required>
                                <option value="A" <?php echo $rol['rol_status'] == 'A' ? 'selected' : ''; ?>>Activo</option>
                                <option value="I" <?php echo $rol['rol_status'] == 'I' ? 'selected' : ''; ?>>Inactivo</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <input type="submit" value="Actualizar Rol" class="btn btn-primary">
                            <a href="index.php?controller=rol&action=index" class="btn btn-secondary">Volver</a>
                        </div>
                    </form>
                        ...
                        <script>
                            document.addEventListener("DOMContentLoaded", function() {
                                Swal.fire({
                                    ...
                                    text: '<?= $success ? "✅ Rol actualizado exitosamente." : "❌ Hubo un error al actualizar el rol. Inténtalo de nuevo." ?>',
                                    icon: '<?= $success ? "success" : "error" ?>',
                                    confirmButtonText: 'Aceptar',
                                    confirmButtonColor: '<?= $success ? "#3085d6" : "#d33" ?>'
                                }).then(() => {
                                    if (<?= $success ? "true" : "false" ?>) {
                                        window.location.href = "index.php?controller=rol&action=index";
                                    }
                                });
                            });
                        </script>
