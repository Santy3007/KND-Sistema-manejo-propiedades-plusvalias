...
<head>
    ...
    <title>PlusvaliaBlk - Gestión de Instituciones Financieras</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800">Gestión de Instituciones Financieras</h1>
                    <a href="index.php?controller=fininstitucion&action=create" class="btn btn-primary mb-3">Crear Nueva Institución</a>
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?php echo htmlspecialchars($institucion['fin_id']); ?></td>
                                <td><?php echo htmlspecialchars($institucion['fin_nombre']); ?></td>
                                <td><?php echo htmlspecialchars($institucion['fin_tasa_interes']); ?>%</td>
                                <td><?php echo nl2br(htmlspecialchars($institucion['fin_terminos'])); ?></td>
                                <td>
                                    <a href="index.php?controller=fininstitucion&action=edit&id=<?php echo $institucion['fin_id']; ?>" class="btn btn-warning">Editar</a>
                                    <a href="javascript:void(0);" class="btn btn-danger btn-delete" data-id="<?php echo $institucion['fin_id']; ?>">Eliminar</a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    ...
                    <script>
                        document.querySelectorAll('.btn-delete').forEach(button => {
                            button.addEventListener('click', function () {
                                const finId = this.getAttribute('data-id');
                                Swal.fire({
                                    title: '¿Estás seguro?',
                                    ...
                                    if (result.isConfirmed) {
                                        window.location.href = `index.php?controller=fininstitucion&action=delete&id=${finId}`;
                                    }
                                });
                            });
                        });
